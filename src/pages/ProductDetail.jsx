...
import { ArrowLeft, ArrowRight, Check, ChevronLeft, ChevronRight, MessageCircle, ShoppingBag, ZoomIn } from 'lucide-react';
...
function ProductView({ product, products, catalogReturn, cart }) {
  ...
  const related = products.filter(item => item.id !== product.id && !isSoldOut(item) && (item.category === product.category || item.material === product.material))
  ...
  const purchaseActions = <div className="product-purchase-actions"><a className="shop-button" href={whatsappLink} target="_blank" rel="noopener noreferrer"><MessageCircle size={19} />{label}</a>{!soldOut && <button type="button" className="shop-button shop-button-secondary" disabled={atLimit} onClick={() => cart.add(product)}><ShoppingBag size={19} />{atLimit ? 'Ya en tu carrito' : 'Agregar al carrito'}</button>}</div>;
  return <div className="product-page shop-shell">
    ...
    {related.length > 0 && <section className="product-related"><div className="product-related-heading"><div><p className="shop-eyebrow">SIGUE DESCUBRIENDO</p><h2>También pueden ser muy tú</h2></div><Link to={catalogReturn} className="shop-text-link">Ver colección <ArrowRight size={16} /></Link></div><div className="shop-product-grid">{related.map(item => <JewelryCard key={item.id} product={item} />)}</div></section>}
    ...
    <div className="product-mobile-purchase product-mobile-cart-actions">{purchaseActions}</div>
  </div>;
}
...
export default function ProductDetail({ products, error = '', cart }) {
  ...
  return <ProductView key={product.id} product={product} products={products} catalogReturn={catalogReturn} cart={cart} />;
}
